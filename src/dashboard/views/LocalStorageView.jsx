...
export default function LocalStorageView() {
  ...
  const handleClear = async () => {
    if (!confirm('Clear all local storage items? This cannot be undone.')) return;
    try {
      const response = await sendMessageToAgent({
  ...
  if (!isAttached) {
    return (
      <EmptyState
        icon="💾"
        title="Not Attached"
        message="Attach to a tab to view and edit local storage"
      />
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Local Storage</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Origin-scoped storage for {attachedTab?.origin}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message={searchTerm ? 'No items match your search' : 'Local storage is empty'}
        />
      ) : (
        <StorageTable
          items={filteredItems}
